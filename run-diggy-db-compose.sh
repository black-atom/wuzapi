...
# Run and manage the services defined in diggy-db-docker-compose.yml
# Usage examples:
#   ./run-diggy-db-compose.sh            # default: up -d
#   ./run-diggy-db-compose.sh up --build
#   ./run-diggy-db-compose.sh down
#   ./run-diggy-db-compose.sh restart
#   ./run-diggy-db-compose.sh logs
#   ./run-diggy-db-compose.sh ps

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

COMPOSE_FILE="${COMPOSE_FILE:-diggy-db-docker-compose.yml}"
...
Environment:
  COMPOSE_FILE     Override compose file (default: diggy-db-docker-compose.yml)
